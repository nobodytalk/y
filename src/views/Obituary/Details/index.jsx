...
const Details = (props) => {
    ...
    const obi_relation = (obi_mourner ? obi_mourner.map((item,key) => (item.relation)) : '')
    let reduce_relation = (obi_relation ? obi_relation.reduce(( a, b ) => {
    ...
    return (
        <>
            <Helmet>
                ...
                <meta property="og:url" content={"https://app.yepum.co.kr/obituary/"+params.obituary_ID} />
                <meta property="og:type" content="website" />
                <meta property="og:image" content="https://app.yepum.co.kr/obituary_og_image_.png" /> 
                <meta property="og:description" content="상주와 장례정보를 확인해세요." /> 
            </Helmet>
            <MainContainer>
                <Wrapeer>
                    ...
                    <Header obiClassName="obituary-header" type="secondary" onHome={isfront == 'front' ? 0 : 1} onBack={isfront == 'front' ? 0 : 1} title={funeral_item.post_title ? funeral_item.post_title : ''} to={ismypage == 'mypage' ? (me_item.obituary_count ? "/obituary/list/" : '/') : '/search'} background="#24243a" color="white" border="rgba(242, 242, 242, 0.1)"/>
                    <DetailsTitle>
                        <img className="aa" src={process.env.PUBLIC_URL + '/logo.png'} alt="" />
                        <h1>故<strong>{obi_item.deceased ? obi_item.deceased : ''}</strong>님의 <br/>추모관</h1>
                        <p><span>향년 {obi_item.age ? obi_item.age+'세' : ''}</span><br/>{obi_item.death_date_text ? obi_item.death_date_text+' 별세' : ''}</p>
                    </DetailsTitle>
                    {gallery_total.total ? 
                    <GalleryWrapper>
                        <Gallery>
                            ...
                            <GalleryContent total={gallery_total.total - 3}>
                                <GalleryListWrapper>
                                    <SlickWrap>
                                        <Slider {...slickSettings} ref={slider => (main_slide_obj.current = slider)}>
                                                {gallery ? gallery.map((item, key) => {
                                                    return (
                                                        <GalleryList url={item.image_url ? item.image_url : ''} onClick={ToggleGallery} data-initial={key}/>
                                                    )
                                                }) : ''}
                                        </Slider>
                                    </SlickWrap>
                                    {!slidePlay ? <a href="javascript:;" onClick={doSlideShow} class="play"><i></i></a>: ''}
                                </GalleryListWrapper>
                            </GalleryContent>
                            {/*                             
                            <GalleryContent total={gallery_total.total - 3}>
                    ...
                    <DetailsWrapper>
                        <DetailsContent>
                        <h2>상주정보</h2>
                        {reduce_relation ? reduce_relation.map((item,key) => {
                            var com = 0;
                            return (
                                <Row>
                                    <Col flex="1"><p>{item ? item : ''}</p></Col>
                                    <Col flex="3">
                                        <span>
                                            {item ? obi_mourner.map((name_item,key) => {
                                                return item == name_item.relation ? ( <>
                                                    {(com++ ? ', ' : '') + name_item.name}
                                                    <a href={"tel:"+name_item.phone}><i></i></a>
                                                </>
                                                ) : '';
                                            }) : ''}
                                        </span>
                            ...
                            <Row>
                                <Col flex="1"><p>빈소</p></Col>
                                <Col flex="3"><span>{obi_item.funeral_home ? obi_item.funeral_home : ''}</span></Col>
                            </Row>
                            <Row>
                                <Col flex="1"><p>입관</p></Col>
                                <Col flex="3">
                                    <span>
                            ...
                            <Row>
                                <Col flex="1"><p>발인</p></Col>
                                <Col flex="3">
                                    <span>
                                        {obi_item.borne_out_date ? obi_item.borne_out_date_text : ''}
                                    </span>
                                </Col>
                            </Row>
                            <Row>
                                <Col flex="1"><p>장지</p></Col>
                                <Col flex="3"><span>{obi_item.burial_plot ? obi_item.burial_plot : ''}</span></Col>
                            </Row>
                        </DetailsContent>
                    </DetailsWrapper>
                    <MapWrapper>
                        <MapView>
                            ...
                            <MapLocation>
                                <a target="_blank" href={"https://map.kakao.com/link/to/"+encodeURI(funeral_item.post_title)+","+funeral_item.y+","+funeral_item.x}></a>
                                {funeral_item.y ? (<KakaoMap lat={funeral_item.y ? funeral_item.y : ''} lng={funeral_item.x ? funeral_item.x : ''}/>) : ''}
                            </MapLocation>
                            <MapContent>
                                <p onClick={CopyAccount}>
                                    <img src={process.env.PUBLIC_URL + '/img/detailsimg1.png'} alt=""/>
                                    {funeral_item.address ? funeral_item.address : ''}
                                    {funeral_item.address ? (
                                        <Hidden>
                                            <input type="text" ref={copy_input} name="copy_account" value={funeral_item.address}/>
                                        </Hidden>
                                    ) : ''}
                                </p>
                            </MapContent>
                            <MapSubContent>
                                <div onClick={showBusPopup} >
                                    <img src={process.env.PUBLIC_URL + '/img/detailsimg2.png'} alt=""/>
                                    <p>대중교통</p>
                                </div>
                                <div onClick={showParkingPopup}>
                                    <img src={process.env.PUBLIC_URL + '/img/detailsimg3.png'} alt=""/>
                                    <p>주차안내</p>
                                </div>
                                <div onClick={showNevPopup}>
                                    <img src={process.env.PUBLIC_URL + '/img/detailsimg4.png'} alt=""/>
                                    <p>네비게이션</p>
                                </div>
                            </MapSubContent>
                        </MapView>
                    ...
                    <MessageWrapper>
                        <Message>
                            <MessageTitle>
                                <h1>조문메시지</h1> <span>{total.total ? total.total : '0'}</span>
                            ...
                            {comments ? comments.slice(0,3).map((item, key) => {
                                return (
                                    <MessageContent>
                                        <div>
                                            <h2>{item.comment_author_name ? item.comment_author_name.replace('(주)예품 임직원','예품 임직원') : ''}</h2><span>{item.comment_date ? item.comment_date : ''}</span>
                            ...
                            {comments ? (
                                <Link to={(isfront == 'front' ? '/front' : '')+"/obituary/"+params.obituary_ID+"/messagelist/"}>
                                    <MessageSubContent>
                                        <h3>조문 메시지 모두 보기</h3>
                                    </MessageSubContent>
                                </Link>
                        ...
                        <UserWrapper>
                            ...
                            <UserContent>
                                {obi_item.flower_order_list.map((order_item) => (
                                    <div>
                                        <img src={order_item.profile_image ? order_item.profile_image : ''} alt="" />
                                        <h2>{order_item.display_name ? order_item.display_name : ''}</h2>
                                    </div>
                                ))}
                            </UserContent>
                        </UserWrapper>
                    ...
                    : ''}
                    <DetailsFooter>
                    {obi_item ? (!obi_item.is_borne_out ? 
                        <FooterButton>  
                            <ButtonGroup>
                                <Button type="secondary" size="medium" width="31.33%" borderColor="#d2d2d2" bgColor="#fff" borderRadius="6px" onClick={obi_item.indemnity_count ? (e)=> window.location.href=(isfront == 'front' ? '/front' : '')+"/obituary/"+params.obituary_ID+"/indemnity" : showIndemNullPopup}>부의금</Button>     
                                <Button type="secondary" size="medium" width="31.33%" borderColor="#d2d2d2" bgColor="#fff" borderRadius="6px" to={(isfront == 'front' ? '/front' : '')+"/shop/obituary/"+params.obituary_ID}>화환보내기</Button>
                                <Button onClick={showSharePopup} data_id={obi_item.ID} size="medium" width="31.33%" borderColor="#d2d2d2" bgColor="#fff" borderRadius="6px">부고알리기</Button>
                            </ButtonGroup>
                        </FooterButton> : '') : ''}
                    </DetailsFooter>
                    <GallerySlickWrapper isopen={GallerySlick}>
                        {GallerySlick && (<Header onBack onClick={ToggleGallery} color="#fff" title="예품 추모관" background="#000" border="rgba(242, 242, 242, 0.1)"/>)}
                        <SlickWrap>
                            <Slider {...settings} ref={slider => (slide_obj.current = slider)}>
                                {gallery ? gallery.map((item,key) => (
                    ...
                    <BusPopup details={funeral_details} show={BusPopup_isshow} onClose={showBusPopup} items={funeral_item} items_detail={funeral_details}/>
                    <ParkingPopup show={ParkingPopup_isshow} onClose={showParkingPopup} items={funeral_item} items_detail={funeral_details} items_parking={funeral_parking}/>
                    <NevPopup show={NevPopup_isshow} onClose={showNevPopup} items={funeral_item} items_detail={funeral_details}/>
                    <SharePopup show={SharePopup_isshow} onClose={showSharePopup} />
                    <IndemNullPopup show={IndemNullPopup_isshow} onClose={showIndemNullPopup} />
                </Wrapeer>
            </MainContainer>
        </>
    );
