...
const AccountAdd = (props) => {
    ...
    const [obi_item, set_obi_item] = useState('');
        ...
        if(!mourner_name){
            ...
            .then((response) => {
                if(response.data.code == '200') {
                    ...
                    set_mourner_KEY(0);
                    set_obi_item(response.data.body.indemnity_count);
                    set_account_name(response.data.body.mourner[0].name);
    ...
    const Submit = (e) => {
        ...
        .then((response) => {
            if(response.data.code == '200') {
                ...
                //결제 필요 여부
                if(response.data.body.need_payment) {
                    window.location.href='/obituary/'+params.obituary_ID+'/indemnity/accountpayment/'+response.data.body.indemnity_ID;
                } else {
                    window.location.href='/obituary/'+params.obituary_ID+'/indemnity/addcomplete';
                }
            }
        })
        .catch((error) => console.log(error))
    ...
    return (
        <MainContainer>
            <Header title={"부의금 " + (obi_item ? '계좌 추가' : '서비스 신청')} 
                    onBack to={"/obituary/"+params.obituary_ID+"/indemnity/account"} background="white"/>
                <Wrapper>
                    ...
                    <InputBind>
                            <Label>예금주</Label>
                            <Input type="textbox" placeholder="상주명과 동일" readOnly value={account_name} onChange={(e) => set_account_name(e.target.value)}/>
                        </InputBind>
                    <InputBind>
                        <Label>은행</Label>
                    ...
                    <InputBind>
                        <Label>계좌번호</Label>
                        <Input type="number" placeholder="- 없이 입력" width="50%" onChange={(e) => set_account_number(e.target.value)} readOnly={validate_number}/>
